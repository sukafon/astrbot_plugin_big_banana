...
“常规配置”中的“允许提供商返回内网媒体 URL”同时作用于提供商返回的图片和视频下载，默认关闭。关闭时，图片和视频都会在请求及每次重定向前检查域名是否解析到公网地址。使用本地服务或代理 Fake-IP 模式且被误判时可以开启，开启后请确保所用提供商可信。LLM 工具输入的参考图片 URL 仍由独立的 `llm_tool_restrict_private_network` 控制。`--url true` 只返回链接，不触发视频下载。
